...
@section('title', __('trans.edit_deep_cleaning'))

@section('content')
<div class="form-container">
    <div class="glass-card form-card">
        <div class="form-header">
            <h3>{{ __('trans.edit_deep_cleaning') }}</h3>
            <p>{{ __('trans.update_deep_cleaning_information_and_media') }}</p>
        </div>
        <form action="{{ route('dashboard.deep_cleanings.update', $deepCleaning) }}" method="POST" enctype="multipart/form-data" class="user-form">
            @csrf
            @method('PUT')
            <div class="form-grid">
                    ...
                    <div class="input-wrapper">
                        <select name="chalet_id" class="form-input @error('chalet_id') error @enderror">
                            ...
                            @foreach($chalets as $chalet)
                                <option value="{{ $chalet->id }}" @if(old('chalet_id', $deepCleaning->chalet_id)==$chalet->id) selected @endif>{{ $chalet->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="input-wrapper">
                        <select name="cleaner_id" class="form-input @error('cleaner_id') error @enderror">
                            ...
                            @foreach($cleaners as $cleaner)
                                <option value="{{ $cleaner->id }}" @if(old('cleaner_id', $deepCleaning->cleaner_id)==$cleaner->id) selected @endif>{{ $cleaner->name }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="form-group">
                    ...
                    <div class="input-wrapper">
                        <input type="date" name="date" class="form-input @error('date') error @enderror" value="{{ old('date', $deepCleaning->date) }}">
                    </div>
                ...
                <div class="form-group">
                    ...
                    <div class="input-wrapper">
                        <input type="text" name="price" class="form-input @error('price') error @enderror" value="{{ old('price', $deepCleaning->price) }}" placeholder="{{ __('trans.enter_price') }}">
                    </div>
                ...
                <div class="form-group">
                    ...
                    <div class="input-wrapper">
                        <textarea name="notes" class="form-input @error('notes') error @enderror" placeholder="{{ __('trans.enter_notes') }}">{{ old('notes', $deepCleaning->notes) }}</textarea>
                    </div>
                ...
                <div class="form-group">
                    ...
                    <div class="input-wrapper">
                        @foreach($deepCleaning->images->where('type','before') as $img)
                ...
                <div class="form-group">
                    ...
                    <div class="input-wrapper">
                        @foreach($deepCleaning->images->where('type','after') as $img)
                            <img src="{{ asset('storage/' . $img->image) }}" style="width:40px;height:40px;border-radius:8px;object-fit:cover;" alt="img">
                        @endforeach
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="input-wrapper">
                        @foreach($deepCleaning->videos->where('type','before') as $vid)
                ...
                <div class="form-group">
                    ...
                    <div class="input-wrapper">
                        @foreach($deepCleaning->videos->where('type','after') as $vid)
                            <a href="{{ asset('storage/' . $vid->video) }}" target="_blank">{{ __('View') }}</a><br>
                        @endforeach
                    </div>
                </div>
                @php $allInventory = \App\Models\Inventory::all(); $usedInventory = $deepCleaning->inventory; @endphp
                    ...
                    <table class="w-full border rounded bg-white">
                        ...
                        <tbody id="inventory-rows">
                            ...
                            <tr>
                                ...
                                <td>
                                    <input type="number" min="1" name="inventory[{{ $idx }}][qty]" class="form-input inventory-qty" placeholder="{{ __('trans.qty') }}" value="{{ $used->pivot->quantity }}" required>
                                </td>
                                <td>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    .use(Uppy.XHRUpload, {
        endpoint: "{{ route('dashboard.deep_cleanings.uploadVideo') }}",
        fieldName: 'video',
        formData: true,
        bundle: false,
        headers: {
