...
@section('title', __('trans.add_deep_cleaning'))

@section('content')
<div class="form-container">
    <div class="glass-card form-card">
        <div class="form-header">
            <h3>{{ __('trans.deep_cleaning_information') }}</h3>
            <p>{{ __('trans.fill_in_the_deep_cleaning_details_below') }}</p>
        </div>
        <form action="{{ route('dashboard.deep_cleanings.store') }}" method="POST" enctype="multipart/form-data" class="user-form">
            @csrf
            <div class="form-grid">
