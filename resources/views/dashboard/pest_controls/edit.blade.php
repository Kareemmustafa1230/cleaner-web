...
@section('title', __('trans.edit_pest_control'))

@section('content')
<div class="form-container">
    <div class="glass-card form-card">
        <div class="form-header">
            <h3>{{ __('trans.edit_pest_control') }}</h3>
            <p>{{ __('trans.update_pest_control_information_and_media') }}</p>
        </div>
        <form action="{{ route('dashboard.pest_controls.update', $pestControl) }}" method="POST" enctype="multipart/form-data" class="user-form">
            @csrf
            @method('PUT')
            <div class="form-grid">
                    ...
                    <div class="input-wrapper">
                        <select name="chalet_id" class="form-input @error('chalet_id') error @enderror">
                            ...
                            @foreach($chalets as $chalet)
                                <option value="{{ $chalet->id }}" @if(old('chalet_id', $pestControl->chalet_id)==$chalet->id) selected @endif>{{ $chalet->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="input-wrapper">
                        <select name="cleaner_id" class="form-input @error('cleaner_id') error @enderror">
                            ...
                            @foreach($cleaners as $cleaner)
                                <option value="{{ $cleaner->id }}" @if(old('cleaner_id', $pestControl->cleaner_id)==$cleaner->id) selected @endif>{{ $cleaner->name }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="form-group">
                    ...
                    <div class="input-wrapper">
                        <textarea name="description" class="form-input @error('description') error @enderror" placeholder="{{ __('trans.enter_description') }}">{{ old('description', $pestControl->description) }}</textarea>
                    </div>
                ...
                <div class="form-group">
                    ...
                    <div class="input-wrapper">
                        <select name="status" class="form-input @error('status') error @enderror">
                            <option value="pending" @if(old('status', $pestControl->status)=='pending') selected @endif>{{ __('Pending') }}</option>
                            <option value="done" @if(old('status', $pestControl->status)=='done') selected @endif>{{ __('Done') }}</option>
                        </select>
                    </div>
                ...
                <div class="form-group">
                    ...
                    <div class="input-wrapper">
                        <input type="date" name="date" class="form-input @error('date') error @enderror" value="{{ old('date', $pestControl->date) }}">
                    </div>
                ...
                <div class="form-group">
                    ...
                    <div class="input-wrapper">
                        <textarea name="notes" class="form-input @error('notes') error @enderror">{{ old('notes', $pestControl->notes) }}</textarea>
                    </div>
                    @error('notes')<span class="error-message">{{ $message }}</span>@enderror
                </div>
                <div class="form-group">
                        <label class="form-label">{{ __('trans.add_images_before_pest_control') }}</label>
                    <div class="input-wrapper">
                ...
                <div class="form-group">
                    <label class="form-label">{{ __('trans.add_images_after_pest_control') }}</label>
                    <div class="input-wrapper">
                ...
                <div class="form-group">
                    <label class="form-label">{{ __('trans.current_images_before_pest_control') }}</label>
                    <div class="input-wrapper">
                        @foreach($pestControl->images->where('type','before') as $img)
                            <img src="{{ asset('storage/' . $img->image) }}" style="width:40px;height:40px;border-radius:8px;object-fit:cover;" alt="img">
                        @endforeach
                    </div>
                </div>
                <div class="form-group">
                    <label class="form-label">{{ __('trans.current_images_after_pest_control') }}</label>
                    <div class="input-wrapper">
                        @foreach($pestControl->images->where('type','after') as $img)
                            <img src="{{ asset('storage/' . $img->image) }}" style="width:40px;height:40px;border-radius:8px;object-fit:cover;" alt="img">
                        @endforeach
                    </div>
                </div>
                <div class="form-group">
                    <label class="form-label">{{ __('trans.current_videos_before_pest_control') }}</label>
                    <div class="input-wrapper">
                        @foreach($pestControl->videos->where('type','before') as $vid)
                            <a href="{{ asset('storage/' . $vid->video) }}" target="_blank">{{ __('View') }}</a><br>
                        @endforeach
                    </div>
                </div>
                <div class="form-group">
                    <label class="form-label">{{ __('trans.current_videos_after_pest_control') }}</label>
                    <div class="input-wrapper">
                        @foreach($pestControl->videos->where('type','after') as $vid)
                            <a href="{{ asset('storage/' . $vid->video) }}" target="_blank">{{ __('View') }}</a><br>
                        @endforeach
                    </div>
                </div>
